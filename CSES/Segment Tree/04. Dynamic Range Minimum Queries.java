...
public class Main {
  static long lookUp(long segment[], int currNode, int nodeStart, int nodeEnd, int queryStart, int queryEnd) {
    ...
    return Math.min(lookUp(segment, currNode * 2, nodeStart, mid, queryStart, queryEnd),
  ...
  static void update(long segment[], int currNode, int nodeStart, int nodeEnd, int queryStart, int queryEnd,
    ...
    segment[currNode] = Math.min(segment[currNode * 2], segment[currNode * 2 + 1]);
  }
  ...
  public static void main(String[] args) {
    ...
    Arrays.fill(segementArray, Long.MAX_VALUE);
    for (int i = 0; i < size; i++) {
      segementArray[N + i] = sc.nextInt();
    }

    for (int i = N - 1; i >= 1; i--) {
      segementArray[i] = Math.min(segementArray[2 * i], segementArray[2 * i + 1]);
    }
