class Solution {
    ...
    public boolean isValidSudoku(char[][] board) {
        for(int i = 0 ; i < 9 ; i++){
            for(int j = 0 ; j < 9 ; j++){
                if(board[i][j] != '.'){
                    if(!rowChecker(board[i][j], board, i) || !colChecker(board[i][j], board, j) || !subMatrixChecker(board[i][j], board, 3 * (i / 3), 3 * (j / 3))){
                        return false;
                    }
                }
            }
